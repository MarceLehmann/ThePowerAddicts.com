...
const PowerAutomateWorkshopDetailPage: React.FC = () => {
    const { t } = useTranslation();
    const { showModal } = useLanguage();
    const location = useLocation();

    const curriculumWeeks = t('powerAutomateDetail.curriculum.weeks');
    const deliverablesCards = t('powerAutomateDetail.deliverables.cards');
    const faqItems = t('powerAutomateDetail.faq.items');
    const meta = t('powerAutomateDetail.meta');
    ...
    return (
        <>
            <EnhancedMeta 
                ...
                keywords="Power Automate Workshop, Power Automate Schulung, Cloud Flows, Desktop Flows, Power Automate Kurs, Workflow Automation"
            ...
            <div className="bg-brand-light-bg">
                ...
                <div className="bg-brand-light-bg border-b">
                    <div className="container mx-auto px-6 py-2 text-sm text-gray-500">
                        ...
                        <span>{t('powerAutomateDetail.nav')}</span>
                    </div>
                </div>
                ...
                <section className="bg-brand-blue-dark text-white py-24 text-center">
                    <div className="container mx-auto px-6">
                        <h1 className="text-4xl md:text-5xl font-extrabold">{t('powerAutomateDetail.title')}</h1>
                        <ul className="mt-6 max-w-4xl mx-auto space-y-3 text-lg text-gray-300">
                            {t('powerAutomateDetail.heroList').map((item: string, index: number) => (
                                <HeroListItem key={index}>{item}</HeroListItem>
                            ))}
                        </ul>
                        <div className="mt-10">
                            <button onClick={() => showModal('waitingListAutomate')} className="bg-gradient-to-r from-brand-gold to-yellow-500 text-brand-blue-dark font-bold text-xl py-4 px-10 rounded-full transition-all duration-300 transform hover:scale-105 hover:shadow-lg inline-block">
                                {t('powerAutomateDetail.cta')}
                            </button>
                        </div>
                    </div>
                </section>
                ...
                <section className="py-24 gear-bg">
                    <div className="container mx-auto px-6">
                        <AnimatedSection className="text-center">
                            <h2 className="text-3xl md:text-4xl font-bold text-brand-blue-dark">{t('powerAutomateDetail.curriculum.title')}</h2>
                            <p className="mt-4 max-w-3xl mx-auto text-lg text-gray-600">{t('powerAutomateDetail.curriculum.subtitle')}</p>
                        </AnimatedSection>
                ...
                <section className="py-24 bg-brand-light-blue">
                    <div className="container mx-auto px-6">
                        <AnimatedSection className="text-center">
                            <h2 className="text-3xl md:text-4xl font-bold text-brand-blue-dark">{t('powerAutomateDetail.deliverables.title')}</h2>
                            <p className="mt-4 max-w-3xl mx-auto text-lg text-gray-600">{t('powerAutomateDetail.deliverables.subtitle')}</p>
                        </AnimatedSection>
                ...
                <section className="py-24">
                    <div className="container mx-auto px-6">
                        <div className="grid lg:grid-cols-2 gap-12">
                            <AnimatedSection>
                                <h2 className="text-3xl font-bold text-brand-blue-dark">{t('powerAutomateDetail.audience.title')}</h2>
                                <ul className="mt-6 space-y-4 text-gray-700">
                                    {t('powerAutomateDetail.audience.items').map((item: string, index: number) => (
                                        <ListItem key={index}>{item}</ListItem>
                                    ))}
                                </ul>
                            </AnimatedSection>
                            <AnimatedSection>
                                <h2 className="text-3xl font-bold text-brand-blue-dark">{t('powerAutomateDetail.format.title')}</h2>
                                <p className="mt-6 text-gray-700" dangerouslySetInnerHTML={{ __html: t('powerAutomateDetail.format.text') }}></p>
                                <div className="mt-6 bg-brand-light-blue p-4 rounded-lg border-l-4 border-brand-teal">
                                    <p className="text-brand-blue-dark" dangerouslySetInnerHTML={{ __html: t('powerAutomateDetail.format.note') }}></p>
                                </div>
                            </AnimatedSection>
                        </div>
                    </div>
                ...
                <section className="py-24 bg-brand-light-blue">
                    <div className="container mx-auto px-6 max-w-4xl">
                        <AnimatedSection className="text-center">
                            <h2 className="text-3xl md:text-4xl font-bold text-brand-blue-dark">{t('powerAutomateDetail.faq.title')}</h2>
                        </AnimatedSection>
                ...
                <section className="py-24">
                    <div className="container mx-auto px-6 text-center">
                        <AnimatedSection>
                        <h2 className="text-3xl md:text-4xl font-extrabold text-brand-blue-dark">{t('powerAutomateDetail.finalCta.title')}</h2>
                        <p className="mt-4 text-lg max-w-2xl mx-auto text-gray-600 leading-relaxed">{t('powerAutomateDetail.finalCta.subtitle')}</p>
                        <div className="mt-8">
                            <button onClick={() => showModal('waitingListAutomate')} className="bg-gradient-to-r from-brand-teal to-brand-teal-dark text-white font-bold text-xl py-4 px-10 rounded-full transition-all duration-300 transform hover:scale-105 hover:shadow-brand-glow inline-block">
                                {t('powerAutomateDetail.cta')}
                            </button>
                        </div>
                        </AnimatedSection>
...
export default PowerAutomateWorkshopDetailPage;
